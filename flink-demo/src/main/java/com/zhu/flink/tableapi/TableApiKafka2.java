...
/**
 * @Author ZhuHaiBo
 * @Create 2021/7/18 17:04
 */
public class TableApiKafka2 {
    public static void main(String[] args) throws Exception {
        ...
        // 按条件查询
        Table result = tableEnvironment.sqlQuery("select * from kafka_sensor_source where id = 'sensor_1'");
        tableEnvironment.toAppendStream(result, Row.class).print("kafka_result");

        // kafka produce conn
        tableEnvironment.connect(new Kafka().version("0.11").properties(properties).topic("sensor_result"))
                .withSchema(new Schema().field("id", DataTypes.STRING())
                        .field("timestamp", DataTypes.BIGINT())
                        .field("temperature", DataTypes.DOUBLE()))
                .withFormat(new Csv())
                .createTemporaryTable("kafka_sensor_result");
        result.insertInto("kafka_sensor_result");
